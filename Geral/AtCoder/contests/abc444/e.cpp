...
template <class T> struct Seg{
    ...
    void unlazy(int node, int l, int r){
        if(lazy[node]){
            tree[node]=join(lazy[node], tree[node]);
            if(l!=r){
...
signed solve(){
    ...
    set<ll> st;
    vector<ll> arr(n);
    for(ll &i: arr) {cin >> i; st.insert(i); st.insert(i-d); st.insert(i+d);}

    int cnt = 1;
    map<ll,ll> mp;
    for(ll x: st) mp[x] = cnt++;
    ...
    for(int i = 0; i < n; i++){
        ll mn = arr[i]-d, mx = arr[i]+d;
        int l2 = mp[mn], r = mp[mx];
        int last = seg.query(l2, r);
        l = max(l, last);

        ans += i-l+1;
        seg.upd(mp[arr[i]], i+1);
    }
